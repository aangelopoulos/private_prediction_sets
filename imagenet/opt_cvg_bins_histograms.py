...
def plot_histograms(df_list,alpha,Ms,unit,num_calib):
    ...
    for i in range(len(df_list)):
        ...
        axs[1].hist(sizes, np.arange(lofb,rolb+d, d), label=f"M={M/unit:.2f}% of " + r"$\sqrt{n}$", alpha=0.7, density=True)
...
if __name__ == "__main__":
    ...
    Ms = np.floor(np.linspace(0.2*unit,unit**2, 6)).astype(int) # max number of bins
